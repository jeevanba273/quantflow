...
try:
    ...
    print(f"⚖️  Portfolio Weights: {portfolio_weights}")
    ...
    # ==========================================
    print("\n" + "=" * 80)
    print("⚠️  SECTION 3: COMPREHENSIVE PORTFOLIO RISK ANALYSIS")
    print("=" * 80)
    ...
    print(f"📈 Historical Daily Vol: {volatility:.4f} ({volatility*np.sqrt(252):.2%} annual)")
    ...
    print("🎯 KEY FINDINGS:")
    print("-" * 50)
    print(f"✅ Portfolio Sharpe Ratio: {sharpe:.3f} ({'Excellent' if sharpe > 1.5 else 'Good' if sharpe > 1.0 else 'Fair'})")
    print(f"✅ Maximum Drawdown: {max_dd:.2%} ({'Low Risk' if abs(max_dd) < 0.15 else 'Moderate Risk' if abs(max_dd) < 0.25 else 'High Risk'})")
    print(f"✅ Diversification Ratio: {diversification_ratio:.2%} ({'Well Diversified' if diversification_ratio > 0.3 else 'Moderately Diversified'})")
